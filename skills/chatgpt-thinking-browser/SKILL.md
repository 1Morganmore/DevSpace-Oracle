...
The bridge passes only public agbrowse commands. Normal execution uses `web-ai send/poll`; recovery observes the recorded canonical URL and exact owned target before considering session doctor or history adjudication. Deterministic history adjudication may additionally use exact `tabs/new-tab/tab-switch/navigate/snapshot/click/active-tab/web-ai status/web-ai snapshot/text/tab-close` operations. No LLM chooses refs or targets.
...
- Treat a matching `tabs --json` `activeCommand.sessionId` only as diagnostic evidence that the recorded helper may still be active. Empty or unchanged answer text, apparent polling stall, different-looking content, or long elapsed time never authorizes terminating its poll/helper; conversely, a stale helper must never delay immediate completion when the exact URL proves `streaming=false` with a nonempty owned answer.
...
- Recovery and user-stop handling are deterministic state reconciliation, not design or review work. Never invoke `independent-review` for them. Check the recorded exact canonical URL first; one unique live match with `streaming=false` and a nonempty answer completes capture, releases the project lock, and proceeds to exact owned-tab cleanup immediately. Do not wait for a stale poll process, command record, `expiresAt`, doctor result, or repeated operator rerun before this direct check.
...
- Never treat a completed different canonical URL as the current active run, even when its title, answer, target, timestamps, or stale session metadata look related. Automation-repair work and the user's project execution are separate ownership lanes: repair code may inspect only automation state and must not explore, edit, or execute the project workload.
...
- On interruption, inspect the persisted canonical URL first. If its unique exact live target exists, observe that target without navigation; `streaming=false` plus a nonempty owned answer finalizes immediately, while an active answer remains protected.
- Never use `poll --navigate` or session doctor to navigate a run whose canonical URL is already known. Target IDs are locators and ownership evidence, not job identity; a changed target ID may be rebound only by one unique exact canonical URL.
- Only when the canonical URL is missing may recovery inspect the recorded session with `agbrowse web-ai sessions doctor <session> --json` (without `--navigate`) and then invoke `chatgpt_agbrowse_run.py --recover-run <exact-run-dir>` for bounded read-only history adjudication. It opens one owned utility target and matches the run-owned prompt filename. Legacy runs require a high-entropy nonce plus two immutable corroborators from the prompt/output contract.
