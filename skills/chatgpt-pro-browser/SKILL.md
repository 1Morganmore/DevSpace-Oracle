...
- Recover with `agbrowse web-ai sessions doctor <session> --navigate --json` first. If it cannot prove a canonical URL, invoke `chatgpt_agbrowse_run.py --recover-run <exact-run-dir>` for deterministic read-only history adjudication. New runs match their unique `prompt-<run_id>.txt`; legacy runs require a high-entropy nonce plus two immutable corroborators.
