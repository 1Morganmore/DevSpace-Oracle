...
Record the job id, `requested_contract`, `enforced_launch_contract`, iteration budget, status, evidence paths, and advisory boundary when the result materially changes the task. The enforced launch contract is always `gpt-5.6-luna` with `max` reasoning; it records the server argv contract and does not claim an independent backend readback.
