...
4. Submit GPT-5.6.
   - Use GPT-5.6 with the strongest account-declared regular-web reasoning level (`Very High` before `High`). Deep Research follows the same selection receipt. Explicit `High` remains available only for an intentional frozen comparison arm. Never silently downgrade. Historical v1 evidence may retain its recorded variant for recovery only.
