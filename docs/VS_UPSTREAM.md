...
authorize submission by itself. Proof visibility rejects non-positive computed
opacity on the candidate or any ancestor, so a visually hidden stale picker
subtree cannot authorize selection. The prior deployed 0.17.2 hashes
`91c5d356...`, `9583e9b4...`, and `fac49260...` are restored through their exact
`thinkingTime.strict.pre-picker-menu-scope.patch`,
`thinkingTime.strict.pre-picker-button-menu-scope.patch`, and
`thinkingTime.strict.pre-picker-containment-proof.patch` reverse assets before
applying the current hash-gated patch.
