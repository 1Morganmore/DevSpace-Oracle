...
def test_builder_allows_normal_browser_source_and_rejects_real_authorization_header(tmp_path: Path) -> None:
    project = fixture_project(tmp_path)
    source = project / "src" / "browser" / "authorization.md"
    source.parent.mkdir(parents=True)
    source.write_text("Authorization is checked by the browser module.", encoding="utf-8")
    manifest = write_manifest(project, [("AGENTS.md", "rules", 0), ("state.json", "state", 1), ("src/browser/authorization.md", "state", 2)])
    assert packet.build(manifest)["included"][-1]["relative_path"] == "src/browser/authorization.md"
    source.write_text("Authorization: Bearer not-a-real-secret", encoding="utf-8")
    manifest = write_manifest(project, [("AGENTS.md", "rules", 0), ("state.json", "state", 1), ("src/browser/authorization.md", "state", 2)])
    with pytest.raises(packet.PacketError, match="UNSAFE_SECRET_CONTENT"):
        packet.build(manifest)
