...
지휘와 종합모드는 둘 다 웹 GPT가 구현까지 담당하지만 구조가 다릅니다.
지휘는 **웹 제출 1회**로 끝나고 단계·영수증·로컬 게이트가 없습니다.
종합모드는 **단계별로 여러 번 제출**하며 각 단계가 다음 미션과 해시 결합
영수증을 쓰고, 최종 웹 PASS와 로컬 게이트 통과로만 완료됩니다. 종합모드의
구현 단계가 곧 지휘와 같은 일을 하므로, 종합모드가 지휘를 포함하는 관계입니다.

목표와 접근이 이미 정해졌다면 지휘가 빠르고 저렴합니다. 계획을 독립 검토로
통과시켜야 하거나 Pro·Web Multi를 끼워야 하거나 로컬 테스트 통과까지
증명해야 하면 종합모드를 쓰세요. 자세한 비교는
[모드 경계](docs/GLOBAL_CHATGPT_ROUTING.md)에 있습니다.
